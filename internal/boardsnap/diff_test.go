...
func TestCLILeadersCSV(t *testing.T) {
	...
	want := "snapshot: 2025-03-15\n" +
		...
		"1,pelora/pl-text-large,0.64,8,8,true\n" +
		"2,nuvixa/embro-7b,0.63,8,8,true\n" +
		"3,zephira-labs/zpl-mini,0.54,8,8,true\n"
	if out != want {
		t.Fatalf("out:\n%q\nwant:\n%q", out, want)
	}
}
...
func TestCLILeadersJSON(t *testing.T) {
	rc, out, _ := runCLI(t, "leaders", "vireo-code-v1", "--as-of", "2025-09-01", "--complete-only", "--format", "json")
	if rc != 0 {
		t.Fatalf("rc=%d", rc)
	}
...
func TestDiffLeaderboardsRemoval(t *testing.T) {
	// A model present only in the earlier snapshot sorts first (current behavior).
